...
def ProjectionImage(H,src_img,world_plane_img):
    ...
    for i in range(0,np.shape(world_plane_img)[0]-1):
        for j in range(0,np.shape(world_plane_img)[1]-1):
            ...
            xp=xp/xp[2]
            #if((xp[0]>min([t1[0][0],t1[2][0]]))and(xp[0]<max([t1[1][0],t1[3][0]]))and(xp[1]>min([t1[0][1],t1[1][1]]))and(xp[1]<max([t1[2][1],t1[3][1]]))):
...
H1=HomographyMatrix(pts_image1,pts_image2)
H2=HomographyMatrix(pts_image2,pts_image3)
H=np.matmul(H1,H2)

output=ProjectionImage(H,image1,image3)
cv2.imwrite('Projection1on3.jpg',output)
